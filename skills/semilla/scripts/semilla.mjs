...
const cfgPath=join(fruti,'semilla.json'), benchPath=join(fruti,'benchmarks.json');
...
function build(scope='.'){
 ...
 const result={schema:'fruti-semilla/v1',generated_at:new Date().toISOString(),verified_commit:gitSha(),scope:slash(relative(root,abs))||'.',roots:rs,nodes};
...
function graph(){const g=readJson(join(knowledge,'graph.json'),null);if(!g)throw new Error('No hay mapa. Ejecuta: fruti semilla init');return g}
...
try{
 if(cmd==='init'||cmd==='map'){const scope=cmd==='map'?(args[1]||'.'):(flag('--scope','.')||'.');const r=build(scope);console.log(`🌱 mapa listo · ${r.files} archivos · ${r.roots} raíces · ${r.ms.toFixed(0)} ms`);console.log(r.counts)}
 else if(cmd==='sync'){const c=changed();const old=readJson(join(knowledge,'index.json'),{});const scope=old.scope||'.';const r=build(scope);console.log(`🌱 sync · ${c.length} archivos cambiados detectados · mapa verificado en ${r.ms.toFixed(0)} ms`)}
 ...
 else if(cmd==='status'){const c=config(),i=readJson(join(knowledge,'index.json'),null);console.log('🌱 Semilla',c.enabled?'ON':'OFF');if(!i)console.log('Map: NOT INITIALIZED');else{console.log('Map: READY');console.log('Commit:',i.verified_commit||'unknown');console.log('Scope:',i.scope);console.log('Counts:',i.counts)}}
 ...
 else if(cmd==='benchmark'){const sub=args[1],all=readJson(benchPath,[]);if(sub==='start'){const name=args[2]||'unnamed',variant=flag('--variant','semilla');writeJson(join(fruti,'.benchmark-active.json'),{name,variant,started_at:new Date().toISOString(),started_ms:Date.now()});console.log('⏱ benchmark started:',name,variant)}else if(sub==='end'){const a=readJson(join(fruti,'.benchmark-active.json'),null);if(!a)throw new Error('No benchmark activo');const run={...a,ended_at:new Date().toISOString(),duration_ms:Date.now()-a.started_ms,input_tokens:Number(flag('--input-tokens',0)),output_tokens:Number(flag('--output-tokens',0)),tool_calls:Number(flag('--tool-calls',0))};all.push(run);writeJson(benchPath,all);console.log(run)}else if(sub==='report'){const by={};for(const r of all)(by[r.name]??=[]).push(r);for(const [n,rs] of Object.entries(by)){console.log('\n'+n);for(const r of rs)console.log(`  ${r.variant.padEnd(8)} ${(r.duration_ms/1000).toFixed(2)}s input:${r.input_tokens} output:${r.output_tokens} tools:${r.tool_calls}`)}}else help()}
 else help();
