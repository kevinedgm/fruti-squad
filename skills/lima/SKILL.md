...
- **Check semilla's map before exploring the repo.** If `.fruti/knowledge/` exists, query it first (`fruti semilla relations <file>`, `impact`, `orphans`) to learn which surfaces and consumers already exist, then open only the files you need to confirm. The map is an index, not the source of truth: when it disagrees with the code, the code wins and you say so. If there is no map, or `fruti semilla status` reports it OFF, explore as usual.
