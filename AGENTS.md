...
3. Select the owning agent and phase.
4. Read ONLY the references required by that phase. A filename mentioned in an AGENT/SKILL document is not an instruction to load it unless the current phase requires its rules.
...
Deep reference prose remains normative when a rule is ambiguous, disputed, changed, or cannot be evaluated from the compact contract.
...
Never copy whole reference documents into a handoff.
